...
- (void)addSubviews
{
    ...
    _recordLabel = [ViewTool getLabelWith:CGRectMake(_imageView.maxX + topSpace,  topSpace, cellW - _imageView.maxX - 2 * topSpace, [UIView getHeight:20]) WithTitle:@"fhsfdhfjdhfd 水电费" WithFontSize:16.0f WithTitleColor: [UIColor colorWithWhite:0.3 alpha:1] WithTextAlignment:NSTextAlignmentLeft];
    ...
    _placeLabel = [ViewTool getLabelWith:CGRectMake(_placeImage.maxX , _recordLabel.maxY , cellW - _placeImage.maxX, 20) WithTitle:@"南京市鼓楼区山西路世贸大厦1000座1000层" WithFontSize:12.0 WithTitleColor: [UIColor colorWithWhite:0.6 alpha:1] WithTextAlignment:NSTextAlignmentLeft];
    [self.contentView addSubview:_placeLabel];
    
    _signState = [ViewTool getLabelWith:CGRectMake(_recordLabel.x, _placeImage.maxY + 1.5 * topSpace, 80, 15) WithTitle:@"已签到" WithFontSize:11.0 WithTitleColor:[UIColor blueColor] WithTextAlignment:NSTextAlignmentLeft];
    [self.contentView addSubview:_signState];
    
    _timeLabel = [ViewTool getLabelWith:CGRectMake(_signState.maxX + topSpace, _signState.y, 100, _signState.height) WithTitle:@"24：24" WithFontSize:11.0 WithTitleColor:[UIColor blueColor] WithTextAlignment:NSTextAlignmentLeft];
